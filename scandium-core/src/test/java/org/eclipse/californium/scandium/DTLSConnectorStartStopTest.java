...
public class DTLSConnectorStartStopTest {
	...
	private void testStopCallsMessageCallbackOnError(final int pending, final int loops, boolean restart)
		...
		for (int loop = 0; loop < loops; ++loop) {
			...
			client.send(message);
			assertTrue(testLogTag + " loop: " + loop + ", " + pending + " msgs, DTLS handshake timed out after "
					+ MAX_TIME_TO_WAIT_SECS + " seconds", latch.await(MAX_TIME_TO_WAIT_SECS, TimeUnit.SECONDS));
			if (lastServerRemaining > -1) {
				assertThat(testLogTag + " number of server sessions changed!", serverHelper.serverConnectionStore.remainingCapacity(),
